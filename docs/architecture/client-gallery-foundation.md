...
The application now exposes one static-compatible `/clients` shell. For
requests whose browser URL is `/clients/<opaque-gallery-id>`, the hydrated shell
reads the single opaque ID segment from `location.pathname`. It does not read
